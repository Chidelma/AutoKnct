...
def searchQuery(event, context):
    ...
    for auto in list(colMong.find({ 'Model' : { '$regex' : event['query'] } }).sort('Model')):
        results.append(auto)
...
def disableUser(event, context):
    ...
    if event['userType'][0] == 'A':
        ...
        adminUserId = retrieveWorkmailId(event['email'])
...
def addToGarage(event, context):
    doc_ref = db.collection(u'USERS').document(event['id'].decode('utf')).get().to_dict()

    if event['map'] == 'owned':
        doc_ref.update({u'Garage.Owned': firestore.ArrayUnion([event['autoID'].decode('utf-8')])})
    elif event['map'] == 'compared':
        doc_ref.update({u'Garage.Compare': firestore.ArrayUnion([event['autoID'].decode('utf-8')])})
    elif event['map'] == 'shared':
        doc_ref.update({u'Garage.Shared': firestore.ArrayUnion([event['autoID'].decode('utf-8')])})

    return retrieveUserGarage(doc_ref['Garage'], doc_ref['Type'])
...
def removeAuto(event, context):
    ...
    if event['map'] == 'owned':
        doc_ref.update({u'Garage.Owned': firestore.ArrayRemove([event['autoID'].decode('utf-8')])})
    elif event['map'] == 'compared':
        doc_ref.update({u'Garage.Compare': firestore.ArrayRemove([event['autoID'].decode('utf-8')])})
    elif event['map'] == 'shared':
        doc_ref.update({u'Garage.Shared': firestore.ArrayRemove([event['autoID'].decode('utf-8')])})
